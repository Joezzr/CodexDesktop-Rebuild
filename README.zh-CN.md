...
- Codex 后端：固定到 OpenAI Codex `rust-v0.148.0-alpha.15` 的指定提交。
- 构建架构：`x86_64-pc-windows-msvc`。
- 发布形式：目录式免安装版，直接运行 `ChatGPT.exe`，无需安装 MSIX、注册应用包或安装证书。
- 当前产物：`out/Codex-win-x64-0.148.0-alpha.15-shell-26.803.10989.0-account-access-fix-v3/ChatGPT.exe`。
...
```text
out\Codex-win-x64-0.148.0-alpha.15-shell-26.803.10989.0-account-access-fix-v3\ChatGPT.exe
```
